...
export default function GymMembers() {
  ...
  useEffect(() => { loadAll(); loadPlans(); loadClasses() }, [loadAll, loadPlans, loadClasses])
      ...
      {showMembershipForm && (
        <Modal title={editingMembershipId ? 'Editar membresía' : 'Nueva membresía'} onClose={() => setShowMembershipForm(false)}>
          ...
          <ModalFooter
            ...
            saveLabel={editingMembershipId ? 'Guardar cambios' : 'Asignar membresía'}
          />
        </Modal>
      )}
...
function MembershipForm({ form, setForm, plans = [], isEditing = false }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Plan picker — only when creating and plans exist */}
      {!isEditing && activePlans.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">
            Seleccionar plan del gym
          </p>
          <div className="grid gap-2">
            {activePlans.map(plan => {
              const selected = form.planName === plan.name && String(form.monthlyPrice) === String(plan.price)
              return (
                <button
                  key={plan.id}
                  type="button"
                ...
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${selected ? 'bg-emerald-500' : 'bg-gray-100 dark:bg-gray-800'}`}>
                    <Tag className={`w-4 h-4 ${selected ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${selected ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-800 dark:text-white'}`}>
                      {plan.name}
                    </p>
                  ...
                  <span className={`text-sm font-bold shrink-0 ${selected ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-700 dark:text-gray-300'}`}>
          ...
          <div className="mt-3 border-t border-gray-100 dark:border-gray-800 pt-3">
            <p className="text-xs text-gray-400 dark:text-gray-500">O completa los datos manualmente:</p>
          </div>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <FormField label="Precio (CLP)"><Input type="number" {...f('monthlyPrice')} placeholder="25000" /></FormField>
