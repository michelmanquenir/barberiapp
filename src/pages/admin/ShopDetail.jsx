...
function ShopDetail() {
  ...
  // Buscador de productos en bodega
  const [shelfSearch, setShelfSearch]                   = useState('')
  const [shelfSearchScannerOpen, setShelfSearchScannerOpen] = useState(false)
  ...
  const shelfSearchResults = useMemo(() => {
    const q = shelfSearch.trim().toLowerCase()
    if (!q) return []
    return products
      .filter(p => p.shelfSlotId != null)
      .filter(p => {
        const id      = String(p.id ?? '')
        const name    = (p.name ?? '').toLowerCase()
        const barcode = (p.barcode ?? '').toLowerCase()
        return id === q || name.includes(q) || barcode === q
      })
      .slice(0, 8)
  }, [shelfSearch, products])

  const handleShelfSearchSelect = async (product) => {
    setShelfSearch('')
    setHighlightedSlotId(product.shelfSlotId)
    setExpandedShelfId(product.shelfId)
    if (!shelfGrids[product.shelfId]) await loadShelfGrid(product.shelfId)
    setTimeout(() => {
      const el = slotRefs.current[product.shelfSlotId]
      if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 350)
    // Quitar highlight después de 3 segundos
    setTimeout(() => setHighlightedSlotId(null), 3500)
  }
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                  ...
                  {shelves.length > 0 && (
                    <div className="relative mb-5">
                      {shelfSearchScannerOpen && (
                        <BarcodeScanner
                          onDetected={code => { setShelfSearch(code); setShelfSearchScannerOpen(false) }}
                          onClose={() => setShelfSearchScannerOpen(false)}
                        />
                      )}
                      <div className="flex gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            className="w-full pl-9 pr-8 py-2 text-sm border border-indigo-200 dark:border-indigo-800 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-400 placeholder-gray-400"
                          />
                          {shelfSearch && (
                            <button
                              onClick={() => setShelfSearch('')}
                              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition"
                            >
                        ...
                        <button
                          type="button"
                          onClick={() => setShelfSearchScannerOpen(true)}
                          title="Escanear código de barras"
                          className="flex-shrink-0 px-3 py-2 border border-indigo-200 dark:border-indigo-800 rounded-lg bg-white dark:bg-gray-800 hover:bg-indigo-50 dark:hover:bg-indigo-950 transition text-indigo-500"
                        >
                          <ScanLine className="w-4 h-4" />
                        </button>
                      </div>
                      ...
                      {/* Resultados */}
                      {shelfSearch.trim() && (
                        <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-white dark:bg-gray-900 border border-indigo-200 dark:border-indigo-800 rounded-xl shadow-lg overflow-hidden">
                          {shelfSearchResults.length === 0 ? (
                            <div className="px-4 py-3 text-sm text-gray-400 dark:text-gray-500 flex items-center gap-2">
                              <Search className="w-4 h-4 opacity-50" />
                              No se encontró ningún producto en bodega con ese criterio
                            </div>
                          ) : (
                            shelfSearchResults.map(p => (
                              <button
                                key={p.id}
                                onClick={() => handleShelfSearchSelect(p)}
                                className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition text-left"
                              >
                                {p.imageUrl
                                  ? <img src={p.imageUrl} alt={p.name} className="w-8 h-8 rounded-lg object-cover border border-indigo-100 dark:border-indigo-800 flex-shrink-0" />
                                  : <div className="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center flex-shrink-0"><Package className="w-3.5 h-3.5 text-indigo-400" /></div>
                                }
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium text-gray-900 dark:text-gray-50 truncate">{p.name}</p>
                                  <p className="text-xs text-indigo-500 dark:text-indigo-400">
                                    {p.shelfName} · Posición <span className="font-mono font-bold">{p.shelfSlotCode}</span>
                                    {p.barcode && <span className="text-gray-400 ml-1">· {p.barcode}</span>}
                                  </p>
                                </div>
                                <span className="text-xs font-mono text-gray-300 dark:text-gray-600 flex-shrink-0">#{p.id}</span>
                              </button>
                            ))
                          )}
                        </div>
                                      ...
                                      <div className="flex flex-wrap gap-3">
                                        {colHeaders.map(c => {
                                          ...
                                          return (
                                            <div
                                              key={c}
                                              ref={el => { if (el) slotRefs.current[slot.id] = el; else delete slotRefs.current[slot.id] }}
                                              ...
                                              className={`relative rounded-xl border-2 flex flex-col cursor-pointer transition-all duration-200 select-none ${
                                                isHighlighted
                                                  ? 'border-amber-400 dark:border-amber-500 bg-amber-50 dark:bg-amber-900/30 w-40 shadow-lg shadow-amber-200/50 dark:shadow-amber-900/30 scale-[1.04]'
                                                  : isEmpty
                                            ...
                                            >
                                              {/* Cabecera */}
                                              <div className={`flex items-center justify-between px-3 py-2 rounded-t-[10px] ${isHighlighted ? 'bg-amber-100 dark:bg-amber-800/60' : isEmpty ? 'bg-gray-100 dark:bg-gray-800' : 'bg-indigo-100 dark:bg-indigo-900/70'}`}>
                                                <span className={`font-mono font-bold text-xs tracking-wide ${isHighlighted ? 'text-amber-700 dark:text-amber-300' : isEmpty ? 'text-gray-400 dark:text-gray-500' : 'text-indigo-700 dark:text-indigo-300'}`}>{code}</span>
                                                {!isEmpty && (
