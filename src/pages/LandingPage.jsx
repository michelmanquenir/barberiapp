    ...
    <header className="fixed top-0 inset-x-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur border-b border-gray-200 dark:border-white/10 transition-colors">
      <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-gray-900 dark:text-white font-bold text-xl tracking-tight">WeServ</span>
        </div>

        {/* Desktop nav */}
        <nav className="hidden md:flex items-center gap-6">
          <a href="#consumidores" className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">Para clientes</a>
          <a href="#negocios"     className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">Para negocios</a>
        </nav>
        ...
        {/* Mobile hamburger */}
        <button onClick={() => setOpen(o => !o)} className="md:hidden p-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-white/10 px-5 py-4 flex flex-col gap-3">
          <a href="#consumidores" onClick={() => setOpen(false)} className="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white py-1">Para clientes</a>
          <a href="#negocios"     onClick={() => setOpen(false)} className="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white py-1">Para negocios</a>
          <div className="pt-2 border-t border-gray-100 dark:border-white/10 flex flex-col gap-2">
            <button
              onClick={toggleTheme}
              className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white py-2 transition"
            >
...
const CONSUMER_FEATURES = [
  { icon: CalendarCheck, title: 'Reserva en segundos',    desc: 'Agenda citas en tu negocio favorito sin llamadas ni esperas. Disponible 24/7.' },
  { icon: Star,          title: 'Reseñas verificadas',    desc: 'Lee opiniones reales de otros clientes y elige el profesional perfecto para ti.' },
  { icon: ShieldCheck,   title: 'Sin complicaciones',     desc: 'Recibe recordatorios automáticos y cancela cuando lo necesites sin penalizaciones.' },
]
...
const AVATAR_COLORS = ['bg-blue-600','bg-violet-600','bg-rose-600','bg-amber-600','bg-emerald-600','bg-cyan-600','bg-fuchsia-600','bg-orange-600']
    ...
    Promise.all([
      ...
      list.forEach(shop => {
        api.getShopGallery(shop.id)
          .then(imgs => { if (imgs?.[0]?.imageUrl) setCovers(prev => ({ ...prev, [shop.id]: imgs[0].imageUrl })) })
          .catch(() => {})
      })
...
function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white transition-colors">
      ...
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <section className="relative bg-gray-900 dark:bg-gray-950 pt-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-5 py-24 md:py-36 text-center">
          ...
          <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-10">
            Barberías, estilistas, gimnasios y más. Agenda tu próxima cita en segundos, sin llamadas, sin esperas.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-12">
            {CATEGORIES.map(({ icon: Icon, label }) => (
              <span key={label} className="inline-flex items-center gap-1.5 bg-white/10 text-gray-300 text-sm px-4 py-1.5 rounded-full border border-white/15">
      ...
      {/* ── For consumers ────────────────────────────────────────────────── */}
      <section id="consumidores" className="bg-gray-50 dark:bg-gray-900 py-20 transition-colors">
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {CONSUMER_FEATURES.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="bg-white dark:bg-gray-800 rounded-2xl p-7 border border-gray-100 dark:border-gray-700 transition-colors">
      ...
      {/* ── For businesses ───────────────────────────────────────────────── */}
      <section id="negocios" className="bg-white dark:bg-gray-950 py-20 transition-colors">
        <div className="max-w-7xl mx-auto px-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">
                Desde la gestión de citas hasta el control de inventario y ventas. WeServ te da las herramientas para hacer crecer tu negocio sin complicaciones.
              </p>
      ...
      {/* ── Footer ───────────────────────────────────────────────────────── */}
      <footer className="bg-gray-100 dark:bg-gray-950 border-t border-gray-200 dark:border-white/10 py-8 transition-colors">
        <div className="max-w-7xl mx-auto px-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-gray-900 dark:bg-white rounded-md flex items-center justify-center">
              <Store className="w-3.5 h-3.5 text-white dark:text-gray-900" />
            </div>
            <span className="text-gray-900 dark:text-white font-bold text-sm">WeServ</span>
          </div>
